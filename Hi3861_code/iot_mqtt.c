...
static void MQTTDemoTask(void)
{
    ...
    while (1) {
        ...
        int distance1 = distance/1000.00;
        int distance2;
        if(distance2!=distance){
            (void)sprintf_s(payload, sizeof(payload), "%d",distance1); 
            distance2=distance;
        }
        else
            (void)sprintf_s(payload, sizeof("0"), "0"); 
        ...
        if ((rc = MQTTPublish(&client, "pubtopic", &message)) != 0) {
            ...
            printf("distance=%d,payload=%s\n",distance,payload);
        }
        osDelay(MQTT_DELAY_2000_MS);
    }
}
